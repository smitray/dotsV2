...
source: sitemap
fetched_at: 2026-02-01T09:22:17.351118647-03:00
rendered_js: false
word_count: 428
summary: This document provides comprehensive guidelines for developing stable and compatible Hyprland plugins, focusing on manifest creation, version management, and best practices for plugin development.
tags:
    - plugin-development
    - hyprpm
    - manifest-file
...
## Making your plugin compatible with hyprpm[](#making-your-plugin-compatible-with-hyprpm)
...
`name` and `authors` are required. `commit_pins` are optional. See [commit pins](#commit-pins) for more info.
...
Although Hyprland plugins obviously are not *required* to follow Hyprland’s formatting, naming conventions, etc. it might be a good idea to keep your code consistent. See [`.clang-format`](https://github.com/hyprwm/Hyprland/blob/main/.clang-format) in the Hyprland repo.
